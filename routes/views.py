    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # shortest node by duration in the inclusive range
                nodes = list(AirportRoute.objects.filter(position__gte=lo, position__lte=hi).values('airport_code', 'position', 'duration'))
                import json
                routes_json = json.dumps(nodes)
                if nodes:
                    # find minimum by duration
                    shortest = min(nodes, key=lambda r: r['duration'])
